...
#Also initializes the subgroups from the memberlist after the Pool has been initialized
  ...
  def __init__(self, gid, tmem, tgp, usrs):
...
#************************* PRINT FUNCTIONS ************************
  ...
  def Print(self):                #prints all members in the object
    for person in self.memlist:
      print ("%d %s%s%s%s"%(person.memnum,person.ei, person.sn, person.tf, person.jp))


  def printsub(self):             #calls print on each subgroup, which is same as Print() -- prints all members in object
    itr = 0
    for subgroup in self.grouplist:
      print("Group %d"%itr)
      subgroup.Print()
...
#              print("high :%d" % group2)
              for user1 in range(0,self.totmem//self.totgroup):  
...
#                  print("group %d user %d is introvert: %s" % (group1,user1,self.grouplist[group1].memlist[user1].ei))
                  for user2 in range(0,self.totmem//self.totgroup):
...
#                        print("BALANCED")
      ...
      if(self.grouplist[group1].boolEI == 2):  #if current sublist too few E
#        print("high :%d" % group1)
        for group2 in range(0, self.totgroup):               #analyze everything further
          if(group1 != group2):
            if(self.grouplist[group2].boolEI == 0):  #to find one with too many E
#              print("low :%d" % group2)
              for user1 in range(0,self.totmem//self.totgroup):  
